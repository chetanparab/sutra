      ...
      <div className="surface relative w-full max-w-lg rounded-[calc(var(--radius)*1.5)] p-7">
        ...
        {step === 0 && (
          <div className="anim-rise">
            ...
            <p className="mt-2.5 text-[13px] leading-relaxed text-secondary">
              Point it at a real repository, describe a change, and watch a real loop iterate — Build, run your tests,
              reflect on failures — until it converges on a real branch. You bring the model; the loop is Sutra's.
            </p>
            <p className="mt-3 text-[12px] leading-relaxed text-muted">
              Three quick things first. You can skip and explore the demo anytime.
            </p>
        ...
        {step === 1 && (
          <div className="anim-rise">
            ...
            <h2 className="serif-hero mt-4 font-display text-[22px] font-semibold tracking-[-0.02em]">Connect a model</h2>
            <p className="mt-2 text-[12.5px] leading-relaxed text-secondary">
              Your key is saved in your OS keychain and handed to the engine as an environment variable only — never
              written to disk in the app, never in a command line, never logged.
            </p>
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              {PROVIDERS.map((p) => (
                <button
                  ...
                  className={cn('rounded-[var(--radius)] border p-3 text-left transition-all', provider === p.id ? 'border-accent/45 bg-accent/[0.09]' : 'border-primary/10 hover:border-primary/20')}
                >
        ...
        {step === 2 && (
          <div className="anim-rise">
            ...
            <p className="mt-2.5 text-[12.5px] leading-relaxed text-secondary">
              Open the <span className="font-medium text-primary">Loop → Design</span> tab and use the{' '}
              <span className="font-medium text-primary">Real repo</span> panel: pick a folder, describe the change,
              give your test command, and launch. Three things stay true, always:
            </p>
            <ul className="mt-3 space-y-1.5 text-[12px] text-secondary">
              {[
                'The loop only touches the folder you pick.',
                'It runs your test command only after you consent.',
